...
/// Convenience wrappers for common targets
pub fn is_comment_attached_to_next_variant(
    children: &[NodeOrToken<SyntaxNode, SyntaxToken>],
    comment_idx: usize,
) -> bool {
    is_comment_attached_to_next_item(children, comment_idx, &[SyntaxKind::VARIANT])
}

pub fn is_comment_attached_to_next_field(
    children: &[NodeOrToken<SyntaxNode, SyntaxToken>],
    comment_idx: usize,
) -> bool {
    is_comment_attached_to_next_item(children, comment_idx, &[SyntaxKind::RECORD_FIELD])
}

/// Get trailing comment on the same line after an item (e.g. after a variant token)
pub fn get_trailing_comment(
    children: &[NodeOrToken<SyntaxNode, SyntaxToken>],
    item_idx: usize,
) -> Option<String> {
