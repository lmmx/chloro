...
/// unresolved goal `T = U`.
pub fn could_unify<'db>(db: &'db dyn HirDatabase, env: Arc<TraitEnvironment<'db>>, tys: &Canonical<'db, (Ty<'db>, Ty<'db>)>) -> bool {
...
pub fn could_unify_deeply<'db>(db: &'db dyn HirDatabase, env: Arc<TraitEnvironment<'db>>, tys: &Canonical<'db, (Ty<'db>, Ty<'db>)>) -> bool {
    could_unify_impl(db, env, tys, |ctxt| ctxt.evaluate_obligations_error_on_ambiguity())
}

fn could_unify_impl<'db>(db: &'db dyn HirDatabase, env: Arc<TraitEnvironment<'db>>, tys: &Canonical<'db, (Ty<'db>, Ty<'db>)>, select: for<'a> fn(&mut ObligationCtxt<'a, 'db>) -> Vec<NextSolverError<'db>>) -> bool {
...
impl<'db> InferenceTable<'db> {
    ...
    pub(crate) fn new(db: &'db dyn HirDatabase, trait_env: Arc<TraitEnvironment<'db>>, owner: Option<DefWithBodyId>) -> Self {
    ...
    pub(crate) fn resolve_with_fallback<T>(&mut self, t: T, fallback_ty: &mut dyn FnMut(DebruijnIndex, InferTy) -> Ty<'db>, fallback_const: &mut dyn FnMut(DebruijnIndex, InferConst) -> Const<'db>, fallback_region: &mut dyn FnMut(DebruijnIndex, RegionVid) -> Region<'db>) -> T
    where
        T: TypeFoldable<DbInterner<'db>>, {
    ...
    pub(crate) fn instantiate_canonical<T>(&mut self, canonical: rustc_type_ir::Canonical<DbInterner<'db>, T>) -> T
    where
        T: rustc_type_ir::TypeFoldable<DbInterner<'db>>, {
    ...
    pub(crate) fn fill_rest_fresh_args(&self, def_id: SolverDefId, first: impl IntoIterator<Item = GenericArg<'db>>) -> GenericArgs<'db> {
    ...
    #[tracing::instrument(skip_all)]
    pub(crate) fn run_in_snapshot<T>(&mut self, f: impl FnOnce(&mut InferenceTable<'db>) -> T) -> T {
    ...
    #[tracing::instrument(level = "debug", skip(self))]
    fn register_obligation_in_env(&mut self, goal: next_solver::Goal<'db, next_solver::Predicate<'db>>) {
    ...
    pub(crate) fn callable_sig(&mut self, ty: Ty<'db>, num_args: usize) -> Option<(Option<FnTrait>, Vec<Ty<'db>>, Ty<'db>)> {
    ...
    fn callable_sig_from_fn_trait(&mut self, ty: Ty<'db>, num_args: usize) -> Option<(FnTrait, Vec<Ty<'db>>, Ty<'db>)> {
...
mod resolve_completely {
    ...
    impl<'a, 'db> Resolver<'a, 'db> {
        pub(super) fn new(ctx: &'a mut InferenceTable<'db>, should_normalize: bool, nested_goals: &'a mut Vec<Goal<'db, Predicate<'db>>>) -> Resolver<'a, 'db> {
            Resolver { ctx, nested_goals, should_normalize }
        }

        fn handle_term<T>(&mut self, value: T, outer_exclusive_binder: impl FnOnce(T) -> DebruijnIndex) -> T
        where
