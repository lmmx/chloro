...
impl DefDiagnostic {
    pub(super) fn unresolved_module(container: LocalModuleId, declaration: AstId<ast::Module>, candidates: Box<[String]>) -> Self {
        Self {
            in_module: container,
    ...
    pub(super) fn unresolved_extern_crate(container: LocalModuleId, declaration: AstId<ast::ExternCrate>) -> Self {
        Self {
            in_module: container,
    ...
    pub(super) fn unresolved_import(container: LocalModuleId, id: AstId<ast::Use>, index: Idx<ast::UseTree>) -> Self {
        Self { in_module: container, kind: DefDiagnosticKind::UnresolvedImport { id, index } }
    }

    pub fn macro_error(container: LocalModuleId, ast: AstId<ast::Item>, path: ModPath, err: ExpandErrorKind) -> Self {
        Self { in_module: container, kind: DefDiagnosticKind::MacroError { ast, path, err } }
    }

    pub fn unconfigured_code(container: LocalModuleId, ast_id: ErasedAstId, cfg: CfgExpr, opts: CfgOptions) -> Self {
        Self {
            in_module: container,
    ...
    pub(crate) fn unresolved_macro_call(container: LocalModuleId, ast: MacroCallKind, path: ModPath) -> Self {
        Self { in_module: container, kind: DefDiagnosticKind::UnresolvedMacroCall { ast, path } }
    }

    pub(super) fn unimplemented_builtin_macro(container: LocalModuleId, ast: AstId<ast::Macro>) -> Self {
        Self { in_module: container, kind: DefDiagnosticKind::UnimplementedBuiltinMacro { ast } }
    }

    pub(super) fn invalid_derive_target(container: LocalModuleId, ast: AstId<ast::Item>, id: AttrId) -> Self {
        Self {
            in_module: container,
    ...
    pub(super) fn malformed_derive(container: LocalModuleId, ast: AstId<ast::Adt>, id: AttrId) -> Self {
        Self {
            in_module: container,
