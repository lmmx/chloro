...
impl<'db> IrPrint<ty::TraitPredicate<Self>> for DbInterner<'db> {
    ...
    fn print_debug(t: &ty::TraitPredicate<Self>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        fmt.write_str(&format!("TODO: {:?}", type_name_of_val(t)))
    }
}

impl<'db> IrPrint<rustc_type_ir::HostEffectPredicate<Self>> for DbInterner<'db> {
    fn print(t: &rustc_type_ir::HostEffectPredicate<Self>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        Self::print_debug(t, fmt)
    }

    fn print_debug(t: &rustc_type_ir::HostEffectPredicate<Self>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        fmt.write_str(&format!("TODO: {:?}", type_name_of_val(t)))
    }
}

impl<'db> IrPrint<ty::ExistentialTraitRef<Self>> for DbInterner<'db> {
    fn print(t: &ty::ExistentialTraitRef<Self>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        Self::print_debug(t, fmt)
    }

    fn print_debug(t: &ty::ExistentialTraitRef<Self>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        crate::with_attached_db(|db| {
            let trait_ = t.def_id.0;
            fmt.write_str(&format!(
                "ExistentialTraitRef({:?}[{:?}])",
...
impl<'db> IrPrint<ty::ExistentialProjection<Self>> for DbInterner<'db> {
    fn print(t: &ty::ExistentialProjection<Self>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        Self::print_debug(t, fmt)
    }

    fn print_debug(t: &ty::ExistentialProjection<Self>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        crate::with_attached_db(|db| {
...
impl<'db> IrPrint<ty::ProjectionPredicate<Self>> for DbInterner<'db> {
    fn print(t: &ty::ProjectionPredicate<Self>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        Self::print_debug(t, fmt)
    }

    fn print_debug(t: &ty::ProjectionPredicate<Self>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        crate::with_attached_db(|db| {
...
impl<'db> IrPrint<ty::NormalizesTo<Self>> for DbInterner<'db> {
    ...
    fn print_debug(t: &ty::NormalizesTo<Self>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        fmt.write_str(&format!("TODO: {:?}", type_name_of_val(t)))
    }
}

impl<'db> IrPrint<ty::SubtypePredicate<Self>> for DbInterner<'db> {
    fn print(t: &ty::SubtypePredicate<Self>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        Self::print_debug(t, fmt)
    }

    fn print_debug(t: &ty::SubtypePredicate<Self>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        fmt.write_str(&format!("TODO: {:?}", type_name_of_val(t)))
    }
}
...
impl<'db> IrPrint<ty::CoercePredicate<Self>> for DbInterner<'db> {
    ...
    fn print_debug(t: &ty::CoercePredicate<Self>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        fmt.write_str(&format!("TODO: {:?}", type_name_of_val(t)))
    }
}
...
impl<'db> IrPrint<rustc_type_ir::PatternKind<DbInterner<'db>>> for DbInterner<'db> {
    fn print(t: &rustc_type_ir::PatternKind<DbInterner<'db>>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        Self::print_debug(t, fmt)
    }

    fn print_debug(t: &rustc_type_ir::PatternKind<DbInterner<'db>>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        fmt.write_str(&format!("TODO: {:?}", type_name_of_val(t)))
    }
}
