...
fn write_impl_header<'db>(impl_: &Impl, f: &mut HirFormatter<'_, 'db>) -> Result<(), HirDisplayError> {
    let db = f.db;
    f.write_str("impl")?;
...
fn write_fields<'db>(fields: &[Field], has_where_clause: bool, limit: usize, in_line: bool, f: &mut HirFormatter<'_, 'db>) -> Result<(), HirDisplayError> {
...
fn write_variants<'db>(variants: &[Variant], has_where_clause: bool, limit: usize, f: &mut HirFormatter<'_, 'db>) -> Result<(), HirDisplayError> {
...
fn write_generic_params<'db>(def: GenericDefId, f: &mut HirFormatter<'_, 'db>) -> Result<(), HirDisplayError> {
...
fn write_where_clause<'db>(def: GenericDefId, f: &mut HirFormatter<'_, 'db>) -> Result<bool, HirDisplayError> {
    let (params, store) = f.db.generic_params_and_store(def);
...
fn has_disaplayable_predicates(db: &dyn HirDatabase, params: &GenericParams, store: &ExpressionStore) -> bool {
...
fn write_where_predicates<'db>(params: &GenericParams, store: &ExpressionStore, f: &mut HirFormatter<'_, 'db>) -> Result<(), HirDisplayError> {
    use WherePredicate::*;
...
fn write_trait_header<'db>(trait_: &Trait, f: &mut HirFormatter<'_, 'db>) -> Result<(), HirDisplayError> {
